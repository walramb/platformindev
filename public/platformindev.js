...
(function() {
  var Block, BoggleParticle, BugLady, Cloud, ControlObj, GenericSprite, Grid, Jelly, Layer, ORIGCLICKPOS, PchooParticle, PowerSuit, Skitter, Sky, Sprite, Target, Thug, V, WORLD, achieve, adjustmouseevent, animate, arrclone, arrsansval, audiobaseurl, availableactions, bglayer, blocksatpoint, body, boggletitles, bogimg, bottomcenter, bottomof, bugsprites, cachedimg, cacheflippedimg, camera, cameraoffset, cancelFullScreen, checkcolls, chievs, control, creatingblock, ded, degstorads, down, drawcolls, drawhitboxes, drawoutline, drawsprite, entcenter, entitycount, euthanasia, falltitles, fglayer, fieldgoaltitles, flipimg, fpscounter, hitboxfilter, hitboxlayer, hueshift, hueshiftmemo, isholdingkey, ladybug, launchFullScreen, left, leftof, load, loadimg, logtimecall, looptick, mainloop, makebox, makechievbox, marlsprites, mouseleftuphandler, mousemiddledownhandler, mousemiddleuphandler, mousemovehandler, mouserightdownhandler, murdertitles, normalizekey, parentstage, pausescreen, pausetext, placeshrub, playsound, preloadcontainer, rainbowhuh, randelem, randfloat, randint, randpos, randvec, rectsoverlap, relativetobox, removesprite, render, renderer, right, rightof, save, scale, screensize, settings, skipframes, snapmouseadjust, sourcebaseurl, sources, spritedrawhitbox, spritelayer, stage, targettitles, tex, tickno, ticktimes, tickwaitms, timecall, titlescreen, titlescreentimeout, tmpcanvas, tmpcanvasjq, toggleFullScreen, topof, tt, up,
  ...
  pausetext = new PIXI.Text("go take a bathroom break or maybe have a snack??? its up to you, doge", {
    font: "16px Arial",
    fill: "white"
  });
  ...
  GenericSprite = (function() {
    ...
    GenericSprite.prototype.render = function() {
      return drawsprite(this, this.src, this.pos, false);
    };
  ...
  Skitter = (function(_super) {
    __extends(Skitter, _super);

    function Skitter(pos) {
      this.pos = pos;
      this.src = 'suit.png';
      this.vel = V();
    }

    Skitter.prototype.render = function() {
      var flip, pos, sprit, spriteanchor;
      flip = false;
      this.anchor = V(0.5, 1);
      pos = this.pos;
      spriteanchor = V(6 / 8, 5 / 8);
      sprit = drawsprite(this, this.src, pos, flip, spriteanchor);
      return sprit.rotation = degstorads(90 + randfloat() * 6);
    };
  ...
  Skitter.prototype.tick = function() {
    this.avoidwalls();
    this.gravitate();
  ...
  Thug = (function(_super) {
    ...
    Thug.prototype.render = function() {
      ...
      sprit = drawsprite(this, this.src, pos, flip);
      sprit.anchor.x = anchor.x;
      return sprit.anchor.y = anchor.y;
    };
  ...
  Thug.prototype.gethitbox = function() {
    return new Block(this.pos.x, this.pos.y + 30, 32, 64);
  };

  GenericSprite.prototype.friction = function() {
    return this.vel.x *= 0.9;
  };
  ...
  Thug.prototype.tick = function() {
    ...
    this.gravitate();
    if (this.lifetime === 0 && this.touchingground()) {
      this.src = 'bugthug.png';
    }
    if (this.lifetime > 0) {
      this.lifetime--;
  ...
  Thug.prototype.blockcollisions = function() {
  ...
  drawsprite = function(ent, src, pos, flip, anchor) {
    ...
    img = flip ? cacheflippedimg(src) : cachedimg(src);
  ...
  control.keytapbind('9', function() {
    return scale -= 0.1;
  });

  control.keytapbind('0', function() {
    return scale += 0.1;
  });
  ...
  control.keytapbind('y', function() {
    return toggleFullScreen(renderer.view);
  });

  control.keytapbind('p', function() {
  ...
  Cloud.prototype.render = function() {
    var ent, flip, offset, pos, sprit, src;
    ent = this;
    src = this.src;
    pos = cameraoffset();
    flip = false;
    ...
    return sprit.tilePosition.y = offset.y;
  };
  ...
  blocksatpoint = function(p) {
    return bglayer.filter(function(box) {
