...
(function() {
  var Block, BoggleParticle, BugLady, Cloud, ControlObj, GenericSprite, Jelly, Layer, PchooParticle, PowerSuit, Sky, Sprite, Target, V, WORLD, achieve, animate, arrclone, arrsansval, availableactions, bglayer, body, boggletitles, bogimg, bottomof, bugsprites, cachedimg, cacheflippedimg, camera, cameraoffset, canvascircle, checkcolls, chievs, control, ded, degstorads, down, drawcolls, drawoutline, drawsprite, entcenter, entitycount, falltitles, fglayer, fieldgoaltitles, flipimg, fpscounter, hitboxfilter, hueshift, hueshiftmemo, isholdingkey, ladybug, left, leftof, load, loadimg, logtimecall, looptick, mainloop, makechievbox, marlsprites, murdertitles, normalizekey, parentstage, placeshrub, preloadcontainer, rainbowhuh, randelem, randfloat, randint, randvec, rectsoverlap, removesprite, render, renderer, right, rightof, save, scale, screensize, settings, skipframes, skylayer, sourcebaseurl, sources, spritedrawhitbox, spritelayer, stage, targettitles, tickno, ticktimes, tickwaitms, timecall, tmpcanvas, tmpcanvasjq, topof, tt, up,
  ...
  Jelly = (function(_super) {
    ...
    Jelly.prototype.render = function() {
      var flip;
      flip = tickno % 10 < 5;
      return drawsprite(this, this.src, this.pos, flip);
    };
  ...
  BugLady.prototype.tick = function() {
    ...
    this.jumping = false;
    this.climbing = this.touchingwall();
  ...
  BugLady.prototype.render = function() {
    var flip, offs, pos, src;
    src = this.getsprite();
    flip = this.facingleft;
    offs = V(0, 4);
    pos = offs.vadd(this.pos);
    return drawsprite(this, src, pos, flip);
  };
  ...
  drawsprite = function(ent, src, pos, flip) {
    var img, sprit, tex;
    ...
    if (flip) {
      sprit.position.x = pos.x + tex.width;
    }
    sprit.position.y = pos.y;
    sprit.anchor.x = 0;
    sprit.setTexture(tex);
    return sprit.scale.x = flip ? -1 : 1;
  };
  ...
  Cloud.prototype.render = function() {
    var ent, flip, img, pos, sprit, src, tex;
    ent = this;
    src = this.src;
    pos = cameraoffset();
    flip = false;
  ...
  skylayer.render = function(ctx) {
    ...
    if (settings.somanygrafics) {
      return offset = V(tickno * -0.2, Math.sin(tickno / 200) * 64);
    }
  };
