...
(function() {
  var $deathmsg, BLOCKCREATIONTOOL, Block, BoggleParticle, BugLady, BugMeter, Burd, Cloud, ControlObj, Energy, GenericSprite, Grid, Hat, INIT, Jelly, Lila, ORIGCLICKPOS, PP, PchooParticle, Poly, PowerSuit, ROBOWORLD_INIT, Renderable, Robo, Sky, Sprite, THISFILE, Target, Thug, V, VTOPP, WORLD, WORLDINIT, WORLD_ONE_INIT, achieve, adjustmouseevent, animate, arrclone, arrowdown, arrowleft, arrowright, arrowup, arrsansval, audiobaseurl, availableactions, bglayer, bindingsDOM, blockdata, blocksatpoint, body, bogglescreen, bogimg, bogsprite, bottomcenter, bottomof, camera, cameraoffset, cancelFullScreen, checkcolls, chievs, control, degstorads, down, drawhitboxes, drawsprite, entcenter, entitycount, fglayer, fpscounter, hitboxfilter, hitboxlayer, initsprite, isholdingkey, k, kick, ladybug, lastmodified, launchFullScreen, left, leftof, load, loadblocks, mainloop, makebox, makechievbox, maketablerow, mousemiddledownhandler, mousemiddleuphandler, mousemovehandler, mouserightdownhandler, normalizekey, parentstage, pausescreen, pausetext, placeshrub, playsound, punch, randelem, randfloat, randint, randpos, randtri, randvec, relativetobox, removesprite, render, renderer, right, rightof, save, scale, scatterents, screensize, selectframe, settings, settingsDOM, skipframes, snapmouseadjust, sourcebaseurl, spritelayer, stage, tex, text, tickno, ticktimes, tickwaitms, timecall, titlescreen, tmpcanvas, tmpcanvasjq, toggleFullScreen, topof, tt, up, updatesettingstable, v, worldchoice, xmlwrap, _ref, _ref1, _ref2,
  ...
  Lila.prototype.tick = Robo.prototype.tick = function() {
    ...
    if (this.scampering && Math.abs(this.vel.x) < 3) {
      this.vel.x += this.facingleft ? -1 : 1;
    }
  ...
  scatterents = function(classproto, num) {
    var _i, _results;
    return spritelayer = spritelayer.concat((function() {
      _results = [];
  ...
  ROBOWORLD_INIT = function() {
    ...
    return spritelayer.push(randtri());
  };
  ...
  WORLD.euthanasia = function() {
    ...
    return spritelayer = _.difference(spritelayer, doomedsprites);
  };

  WORLD.tick = function() {
    var key, _base, _i, _len, _ref2;
    _ref2 = control.heldkeys;
  ...
  lastmodified = function(date) {
    return body.prepend("<p>last modified " + (jQuery.timeago(date)) + "</p>");
  };
