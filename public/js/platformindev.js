...
(function() {
  var AKA, BASETOOL, BLOCKCREATIONTOOL, BLOCKPAINT, Block, BoggleParticle, BugLady, BugMeter, Bullet, CARVER, COLLGRIDSIZE, CURRENT_LEVEL, CURSOR, Cloud, ControlObj, DEPRECATE, Damagebox, DoubleJumper, Energy, EnergyMeter, GenericSprite, Goal, Gold, Grid, HUDLAYER, Hero, HitboxRayIntersect, HurtWire, INIT, Lila, Line2d, MAXBUGSPEED, MLEFT, MOVEBLOCKTOOL, MOVETOOL, MRIGHT, MoneyMeter, MoveBlockTool, MoveTool, NOOPTOOL, ORIGCLICKPOS, OnewayBlock, PP, PchooParticle, Pickup, PlaceholderSprite, Platform, PowerSuit, Rect, Renderable, Robo, SCREENCURS, SPAWNERTOOL, SPAWNTOOL, Shieldbubble, Shrub, Smoochie, Spawner, TELEPORTTOOL, TEXBYNAME, THISFILE, TICKLOG, TILESELECT, TRIANGLETOOL, Thug, Tool, Turret, UNIONTOOL, V, VTOPP, WARN, WATERTOOL, WORLD, WORLDINIT, Water, World, XXXX, _CHARbindingnames, _DEFAULTHITBOXSIZE, _Rectmethods, __snap, _bind, _cycle, _dash, _demotick, _doc, _facing, _lastmodified, _loadcurrlevel, _maketiles, _ml, _move, _particle, _repdupes, _sidestep, _toggleFullScreen, _versionfoot, _wheelin, achieve, actioncategories, adjustedscreensize, adjustmouseevent, allactions, alltools, animate, arr, ascii, assert, audiobaseurl, audiocache, audiocachelength, availableactions, b2n, bglayer_remove_block, bindaction, bindingsDOM, bindingsDOMcontainer, blockcarve, blocksatpoint, blocktextures, blocktools, body, bottomcenter, boxesbounding, boxtouchingwall, browserprefix, bts, bugspeed, bugtextures, bugthrust, but, call, camera, cameraoffset, cancelFullScreen, carveoutblock, changelayer, char, checkcolls, ck, classname, closestpoint, control, css, curraudiotrack, cv, dotfilt, down, drawhitboxes, drawsprite, edithistory, entcenter, entitycount, eventelement, firebullet, fixbox, fontmap, fontsrc, fonttexs, fpscounter, getCursorBlocks, get_sprites_of_class, getentsunderpoint, gettileoffs, glyph, highlightoverlaps, hitboxfilter, hitboxfilter_OLD, hitboxlayer, hypesprites, hz, i, infobox, initsprite, is_firefox, isfullscreen, isholdingbound, isholdingkey, j, jame, jump, k, key, kick, ladybug, launchFullScreen, left, len, len1, level_files, load, loadblocks, loadent, loadents, loadlevel, loadlevelfile, loadspawners, mainloop, makebox, maketablerow, maketiles, mktool, mousemiddledownhandler, mousemiddleuphandler, mouserightdownhandler, n, nextlevel, noop, normalizekey, o, objnames, offs, parentstage, pausefunc, pausescreen, pausestring, pausetext, placeshrub, playsound, pointlisttoedges, preload, prevtool, punch, q, randpos, randposrel, randtri, readablebindings, record_l, record_s, recordinputs, ref, ref1, ref2, ref3, relativetobox, removesprite, render, renderer, reserveKeyNamed, reservedkeys, reset_tick_stats, resetstage, restartlevel, results, right, root, save, scale, screensize, screensize_default, selectall, selectframe, setcursor, settings, settingsDOM, settingsDOMcontainer, skipframes, smame, snapmouseadjust, snapmouseadjust_always, snapmouseadjust_down, sourcebaseurl, spawnables, spawnselection, spr, stage, stageremovesprite, stats, tb, textcontainer, tickno, ticktimes, tickwaitms, timecall, tmpcanvas, tmpcanvasjq, tmpinnerstage, tmpstage, toggleFullScreen, tool, tool_clickdelete, toolbar, tt, u, unfuck, up, updateinfobox, updatesettingstable, v, voidcaller, wheel, wspacing, xmlwrap,
    slice = [].slice,
  ...
  for (i = q = 0, len = pausestring.length; q < len; i = ++q) {
    char = pausestring[i];
    offs = V(32, 64);
    wspacing = 16;
  ...
  closestpoint = function(p, pointarr) {
    var closest, len1, point, u;
    closest = pointarr[0];
    for (u = 0, len1 = pointarr.length; u < len1; u++) {
      point = pointarr[u];
  ...
  BugLady.prototype.smashblock = function() {
    var block, blockcandidates, collidebox, len1, results1, u;
    collidebox = this.gethitbox();
    blockcandidates = hitboxfilter(collidebox, WORLD.bglayer);
    results1 = [];
    for (u = 0, len1 = blockcandidates.length; u < len1; u++) {
      block = blockcandidates[u];
  ...
  boxtouchingwall = function(collidebox) {
    var block, blockcandidates, len1, notontop, u;
    blockcandidates = hitboxfilter(collidebox, WORLD.bglayer);
    for (u = 0, len1 = blockcandidates.length; u < len1; u++) {
      block = blockcandidates[u];
  ...
  GenericSprite.prototype.avoidwalls = function() {
    var actualbox, block, blockcandidates, collidebox, jumpthroughable, len1, notontop, results1, u;
  ...
  GenericSprite.prototype.touchingground = function() {
    var block, blockcandidates, collidebox, fromgrid, len1, otherboxes, touch, u;
    touch = false;
    collidebox = this.gethitbox();
  ...
  _repdupes = function(arr, rep) {
    var len2, prev, results1, x, z;
    results1 = [];
    for (z = 0, len2 = arr.length; z < len2; z++) {
      x = arr[z];
      if (x === prev) {
        results1.push(rep);
      } else {
  ...
  WORLD.tick = function() {
    var ACTIVEENTS, base1, len2, ref2, z;
    _demotick();
    reset_tick_stats();
  ...
  blockcarve = function(aa, bb) {
    var a, b, blok, blokx, len2, results1, x1, x2, x3, x4, y1, y2, y3, y4, z;
    ...
    results1 = [];
    for (z = 0, len2 = blokx.length; z < len2; z++) {
      blok = blokx[z];
  ...
  carveoutblock = function(b) {
    var ab, block, bloke, len2, len3, results1, tocarve, todelete, z;
    block = new Block(b.x, b.y, b.w, b.h);
    tocarve = block.allstrictoverlaps();
    for (z = 0, len2 = tocarve.length; z < len2; z++) {
      bloke = tocarve[z];
      blockcarve(bloke, block);
    }
    todelete = block.allstrictoverlaps();
    results1 = [];
    for (ab = 0, len3 = todelete.length; ab < len3; ab++) {
      bloke = todelete[ab];
  ...
  WATERTOOL = _.extend({}, NOOPTOOL, {
    ...
    mousedown: function(e) {
      var adjusted, bl, blocksundercursor, len2, results1, z;
      if (e.button !== 0) {
        return;
      }
      adjusted = adjustmouseevent(e);
      ...
      results1 = [];
      for (z = 0, len2 = blocksundercursor.length; z < len2; z++) {
        bl = blocksundercursor[z];
  ...
  mktool({
    name: "painter",
    action: function(p) {
      var bl, len2, ref4, results1, z;
      ref4 = getCursorBlocks();
      results1 = [];
      for (z = 0, len2 = ref4.length; z < len2; z++) {
        bl = ref4[z];
        bl.tile = TILESELECT;
  ...
  actioncategories["level editing"] = {
    "align all blocks to grid": function() {
      var block, gridsize, len2, ref4, results1, z;
  ...
  _wheelin = function(offs) {
    var bl, len2, ref4, results1, size, z;
